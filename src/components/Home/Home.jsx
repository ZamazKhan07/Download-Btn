...
const Home = () => {
  ...
  const fetchProduct = async () => {
    ...
    resData.forEach(item => {
      // Accessing the price of each object
  ...
  const filterdHomePrice = async () => {
    const minPrice = value[0]
    const maxPrice = value[1]
    const filteredData = await fetchDataDynamic();
    const updatedList = filteredData.map((category) => ({
      ...category,
      categoryList: category.categoryList.filter(
        (data) => data.price >= minPrice && data.price <= maxPrice
      ),
    }));
    setContract(updatedList);
  ...
  const filterdPrice = async () => {
    const minPrice = value[0]
    const maxPrice = value[1]
    const res = await dispatch(AllProductsUserAction());
    let ubdatedList = res?.data?.products.filter(( data ) => data.price >= minPrice && data.price <= maxPrice)
    setProductType(ubdatedList);
  }
      ...
      >
        ...
        <DialogContent>
          ...
          <Box>
            ...
            /> */}
            <Typography>Rs{value}</Typography>
            <Slider
              sx={{
                marginTop: "30px",
                "& .MuiSlider-thumb": {
